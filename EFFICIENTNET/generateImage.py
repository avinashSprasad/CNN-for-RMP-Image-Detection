...
# -------------------- GLCM COMPUTATION FUNCTION --------------------
def compute_glcm_batch(x, num_levels=256, distances=[1], angles=[0]):
    B, C, H, W = x.shape

    # Unnormalize from ImageNet stats
    ...
    for i in range(B):
        ...
        img_gray = rgb2gray(img_np)  # float [H, W]
        ...
        glcm_t = glcm_t.reshape(len(distances)*len(angles), num_levels, num_levels)
...
class CustomEfficientNet(nn.Module):
    def __init__(self, num_classes=2, num_levels=256, distances=[1], angles=[0]):
        super().__init__()
        self.num_levels = num_levels
        self.distances = distances
        self.angles = angles

        glcm_channels = len(distances) * len(angles)
        self.stem = nn.Sequential(
            nn.Conv2d(glcm_channels, 32, 3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        x = compute_glcm_batch(x, self.num_levels, self.distances, self.angles)
        x = self.stem(x)
        x = self.block1(x)
        x = self.pool(x)
        x = self.block2(x)
